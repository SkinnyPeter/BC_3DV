...
def main() -> None:
    ...
    step = 0
    best_val = float("inf")
    action_step_dim = sample["proprio"].numel()
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        for batch in train_loader:
            ...
            batch = move_batch_to_device(batch, device)
            pred = model(batch["image"], batch["proprio"])
            loss = bc_loss(pred, batch["action"], cfg["loss"]["smoothness_weight"], action_step_dim)
        ...
        if epoch % cfg["training"]["val_every"] == 0:
            val_loss = evaluate(model, val_loader, device, cfg["loss"]["smoothness_weight"], action_step_dim)
